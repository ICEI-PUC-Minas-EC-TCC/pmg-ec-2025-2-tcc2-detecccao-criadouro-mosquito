...
if (!fs.existsSync(sourceImage)) {
  ...
  console.error(
    "   A imagem deve ser PNG, 1024x1024 pixels, com fundo transparente.\n"
  );
  process.exit(1);
}
...
async function generateIcons() {
  ...
  try {
    for (const [density, config] of Object.entries(densities)) {
      ...
      console.log(
        `✅ ${density}: ic_launcher_foreground.webp (${config.foreground}x${config.foreground})`
      );
      ...
      console.log(
        `✅ ${density}: ic_launcher.webp (${config.foreground}x${config.foreground})`
      );
